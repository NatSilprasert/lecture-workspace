...
- ถาม: GPU เน้น throughput หมายถึงอะไร?
  ตอบ: เน้นงานรวมต่อเวลา มากกว่าลดเวลา thread เดี่ยว.
- ถาม: SIMT ต่างจาก SIMD แบบมุมมองโปรแกรมเมอร์ยังไง?
  ตอบ: SIMT เขียนเหมือนหลาย threads แต่ฮาร์ดแวร์ issue ร่วมกัน.
- ถาม: warp divergence ทำให้ช้าเพราะอะไร?
  ตอบ: ทาง branch ต่างกันจนต้องรันแยกเส้นทางทีละส่วน.
- ถาม: วิธีลด divergence ที่ใช้ได้บ่อยคืออะไร?
  ตอบ: จัดข้อมูลให้ threads ใน warp มีพฤติกรรมคล้ายกัน.
- ถาม: ทำไม GPU ต้องมี threads จำนวนมาก?
  ตอบ: เพื่อสลับไปทำงานอื่นระหว่างรอ memory latency.
- ถาม: global memory กับ shared memory ต่างกันหลัก ๆ?
  ตอบ: global ใหญ่แต่ช้ากว่า ส่วน shared เล็กแต่เร็วกว่าใน block.
- ถาม: host-device transfer เป็นคอขวดได้เมื่อไร?
  ตอบ: เมื่อย้ายข้อมูลถี่หรือข้อมูลใหญ่จนเวลาย้ายเด่นกว่าคำนวณ.
- ถาม: DMA ช่วยงาน CPU-GPU อย่างไร?
  ตอบ: โอนข้อมูลตรงโดยไม่ให้ CPU คัดลอกเองทุกขั้น.
- ถาม: `warp` กับ `wavefront` ต่างกันอย่างไร?
  ตอบ: แนวคิดคล้ายกัน แต่ `warp` เป็นคำของ NVIDIA ส่วน `wavefront` เป็นคำของ AMD.
- ถาม: Amdahl's Law เตือนอะไรในการ optimize GPU?
  ตอบ: ต้องลด serial part ไม่งั้น speedup จะตันเร็ว.
